...
namespace cr.ffmpeg
{
    public static class DownloadFFmpeg
    {
        ...
        {
            int bits = Environment.Is64BitOperatingSystem ? 64 : 32;
        ...
        public static async Task ExtractTo(string FolderPath)
        {
            await Task.Run(() =>
            {
                using ZipArchive archive = ZipFile.OpenRead(FFmpegArchivePath);
                ZipArchiveEntry ffmpegEntry = archive.Entries.First(M => M.Name == ExeName);
